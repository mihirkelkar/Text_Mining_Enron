...
def parse_email_tokens(cemail):
  ...
  for sent in sent_tokens:
    ...
    bigram_tokens += nltk.ngrams(clean_up(word_tokenize(sent)),2)
...
def main():
  ...
  print "Making bigrams for " +  sys.argv[3] + " block length with " +  sys.argv[2] + " function words"
  #Go change the counter for random.choice in the parse_directory function
  final_counter = {}
  slice = sys.argv[2]
  kp  = open(sys.argv[1] + "/bigrams/" + "bigrams_" + slice + "_" + sys.argv[3], 'w')
  fp = open("../bigram_token_list", "r")
  function_words = list()
  counter = 0
  while counter < 200:
  ...
  for ii in function_words[0:int(slice)]:
    kp.write(ii[0] + " " + ii[1]  +  ","+ str(math.floor(final_counter[ii] / 50)) + "\n")
  fp.close()
  kp.close()
